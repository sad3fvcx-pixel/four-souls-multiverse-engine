    ...
    for group in ("effects", "conditions", "targets"):
        for one in can[group]:
            for field in one["fields"]:
                ...
                elif field["role"] == STRUCTURE and (
                    field["a_list_of"] or field["each_shaped_like"]
                ):
                    # One that says what it holds is described, and goes where
                    # its kind goes — a list of them, or several under names.
                    assert field["shown"] in ("body", "named"), where
                elif field["role"] == STRUCTURE:
                    # Nested data nothing here describes.
