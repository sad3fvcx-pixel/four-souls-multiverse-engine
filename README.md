...
Version 0.1.0, the first release candidate. Python 3.12 or newer, no
dependencies. See the [changelog](CHANGELOG.md).
...
`fsme` opens a browser at the front door: play a game, run a study, test a
card, open a report. Every button on it runs the same function the matching
command runs and prints the same text, because two sources of truth would make
the first disagreement between them unanswerable.
